...
func viewScoreboard(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "index.html", pageData(c, "Scoreboard", gin.H{"scores": teamData, "fullNames": true}))
}
...
func viewImage(c *gin.Context) {
	...
	c.HTML(http.StatusOK, "index.html", pageData(c, "Scoreboard for "+imageName, gin.H{"scores": teamData, "imageFilter": getImage(imageName), "event": sarpConfig.Event, "fullNames": true}))
}
...
func viewSettings(c *gin.Context) {
	c.HTML(http.StatusOK, "settings.html", pageData(c, "settings", gin.H{"scoring": acceptingScores, "isLead": isLeadUser(getUser(c))}))
}
...
func changeSettings(c *gin.Context) {
	...
	if settingType == "announcement" {
		...
		if !isLeadUser(currentUser) {
			err = errors.New("Only the lead account can adjust scores")
		} else {
	...
	c.HTML(http.StatusOK, "settings.html", pageData(c, "settings", gin.H{"scoring": acceptingScores, "isLead": isLeadUser(currentUser), "msg": msg, "err": err}))
}
